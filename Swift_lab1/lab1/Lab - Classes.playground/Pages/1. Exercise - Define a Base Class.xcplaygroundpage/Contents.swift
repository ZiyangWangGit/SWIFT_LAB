...
class Spaceship{
    ...
    // Method to move the spaceship to left by one
    func moveLeft() {
        self.position -= 1
    }

    // Method to move the spaceship to right by one
    func moveRight() {
        self.position += 1
    }
    
    //Method to handle hits situation
    func wasHit(){
        self.health -= 5
        if (self.health <= 0){
...
falcon.moveLeft()
falcon.moveRight()
print("The current position of falcon is: \(falcon.position)")
...
falcon.wasHit()
print("The current value of health for falcon is: \(falcon.health)")
